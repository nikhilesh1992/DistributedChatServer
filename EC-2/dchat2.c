...
void controllerLeader(char *buf, ArrayString *arrayString)
{
	...
	else if(strcmp(arrayString[0].String,"Exists")==0)
	{
		...
		{
			sem_wait(&msgSent);
			insertFairQ(getIDForUser(arrayString[2].String),bufBroadCast);
			sem_post(&msgSent);
...
int getIDForUser(char *User)
{
	int i;
	for(i=0;i<20;i++)
	{
		if(strcmp(User,chatUser[i].Username) == 0)
		{
			return chatUser[i].ID;
		}
	}
	return -1;
}

int getMsgNumForUser(char *User)
{
	int i;
	for(i=0;i<20;i++)
	{
...
void fairQueue(char *msg,InsertionList **head, InsertionList **tail)
{
	char tempbuf[500];
	int priority,ID;
	ArrayString arrayString[20];
	...
	InsertionList *addNode = (InsertionList *)malloc(sizeof(InsertionList));
	strcpy(addNode->msg, msg);
	if((ID = getIDForUser(arrayString[2].String)) != -1)
	{
		priority = (((20-ID) + 1) + ((getMsgNumForUser(arrayString[2].String) - 1)*10));
	}
	addNode->seqNum = priority; //Message~Sam:: Hi~timestamper
	addNode->next = NULL;
	if(*head == NULL && *tail == NULL)
	{
		//printf("first message added out of order\n");
...
char* RemoveNodeFairQ(char *msgbuf, InsertionList **head, InsertionList **tail)
{
	InsertionList *removeNode;
	if( *head == *tail)
	{
...
void insertFairQ(int id,char *msg)
{
	int index = (20 - id);
	QueueNode *addNode=NULL;
	addNode = (QueueNode *)malloc(1*sizeof(QueueNode));
	strcpy(addNode -> content,msg);
	addNode -> next = NULL;
	if( fairQList[index].head == NULL &&  fairQList[index].tail == NULL )
	{
		fairQList[index].head = addNode;
		fairQList[index].tail = addNode;
	}
	else
	{
		(fairQList[index].tail) -> next = addNode;
		fairQList[index].tail = addNode;
	}
}
...
void printFairQ()
{
	...
	for(i=0;i<20;i++)
	{
		if(fairQList[i].head == NULL && fairQList[i].tail == NULL)
		{
			printf("Queue empty for the user\n");
		}
		else
		{
			temp = fairQList[i].head;
			while(temp != NULL)
			{
				printf("%s - ", temp->content);
				temp = temp->next;
...
char* removeFairQ(char *msg,int index)
{
	QueueNode *temp;
	
	if( fairQList[index].head == fairQList[index].tail )
	{
		temp = fairQList[index].head;
		strcpy(msg,temp -> content);
		fairQList[index].head = NULL;
		temp -> next = NULL;
		fairQList[index].tail = NULL;
		free(temp);
	}
	else
	{
		...
		strcpy(msg,temp -> content);
		temp = temp -> next;
		free(fairQList[index].head);
		fairQList[index].head = temp;
	}
	return msg;
}
...
void *threadForLeaderBrdCast(void *arg)
{
	...
	while(1)
	{
		if(amILeader(nameOfUser) == 1)
		{
			for(i = 0; i < 20; i++)
			{
				if(fairQList[i].head != NULL && fairQList[i].tail != NULL && highPriority == 0 && readyToBroadCast() == 1)
				{
					sem_wait(&msgSent);
					strcpy(temp,removeFairQ(temp,i));
					//printf("%s\n",temp);
